function DIR = changeSubDirectoryPaths(DIR,hpStr,window, betaStr,threshold,wavThreshold,version,baseline,muscIL,detrend)

DIR.overallPath = strcat(DIR.SET_PATH,"01-output/hp-",hpStr, "window-", window, "beta", betaStr,"_amp-",int2str(threshold),...
        "_wavThreshold-", wavThreshold, "_version-",int2str(version),"_baseline-", baseline, ...
        "_muscIL-", muscIL,"detrend-", detrend);
