...
# Save
plot(p)
png(file=here("data", "sensitivity_analysis", "A5_sensAnal_BF_simpleGroup-S4.png"),
...
# Not necessary because no effects ----------------------------------------------------
...
ggplot(res, aes(x = prior_sd, y = BF)) +
  ...
  theme(axis.text.y = element_text(size = 8)) +
  ggtitle("Bayes factors for simple effect of Group for Training speaker")
...
ggplot(res, aes(x = prior_sd, y = BF)) +
  ...
  annotate("text", x = tail(prior_sd, n=1) + 20, y = 1 / 30, label = "Evidence in favor of H0", size = 5) +
  theme(axis.text.y = element_text(size = 8)) +
  ggtitle("Bayes factors for effect of nrSpeakersDaily")
