...
data_acq = data_acq_noncentered

# Descriptive Table Demographics
descr_demographics <- data_acq %>%
...
write.table(descr_demographics_t, file = here("data","tables","descriptive_demographics_ACQ.txt"), sep = ",", quote = FALSE, row.names = TRUE, col.names = FALSE)


# Descriptive Table methods
descr_methods_group <- data_acq %>%
  distinct(Subj, .keep_all = TRUE) %>%  # Keep only one observation per subject
  group_by(Group) %>%
  summarise(
...
write.table(descr_methods_group_t, file = here("data","tables","descriptive_methodsgroup_ACQ.txt"), sep = ",", quote = FALSE, row.names = TRUE, col.names = FALSE)

descr_methods_groupTS <- data_acq %>%
...
write.table(descr_methods_groupTS_t, file = here("data","tables","descriptive_methodsgroupTS_ACQ.txt"), sep = ",", quote = FALSE, row.names = TRUE, col.names = FALSE)

descr_methods_trials<- data_acq %>%
  group_by(TestSpeaker, Group) %>%
  filter(TestSpeaker %in% c("S1", "S3", "S4")) %>%
  summarise(
...
write.table(descr_methods_trials_t, file = here("data","tables","descriptive_methodsTrials_ACQ.txt"), sep = ",", quote = FALSE, row.names = TRUE, col.names = FALSE)
