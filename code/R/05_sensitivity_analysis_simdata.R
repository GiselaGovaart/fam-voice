...
## Model orig prior  ------------------------------------------------------------
m_orig <- readRDS(here("data", "model_output", "04_model_posteriorpredcheck_rec.rds"))
...
m_alt_1 <- brm(MMR ~ 1 + TestSpeaker * Group + 
               ...
               cores = num_chains, 
               seed = project_seed,
               file = here("data", "model_output", "05_model_sensitivity_altern1_rec.rds"),
               file_refit = "on_change",
               save_pars = save_pars(all = TRUE)
)
plot(m_alt_1) # looks okay, Rhat is 1.01 in some cases, but the estimates are still good
...
m_alt_2 <- brm(MMR ~ 1 + TestSpeaker * Group + 
               ...
               cores = num_chains, 
               seed = project_seed,
               file = here("data", "model_output", "05_model_sensitivity_altern2_rec.rds"),
               file_refit = "on_change",
               save_pars = save_pars(all = TRUE)
)
plot(m_alt_2) # looks good
...
m_alt_3 <- brm(MMR ~ 1 + TestSpeaker * Group + 
               ...
               cores = num_chains, 
               seed = project_seed,
               file = here("data", "model_output", "05_model_sensitivity_altern3_rec.rds"),
               file_refit = "on_change",
               save_pars = save_pars(all = TRUE)
)
plot(m_alt_3) # looks good

summary(m_orig) # Rhat and ESS'es look good
summary(m_alt_1) # Rhat and ESS'es look good 
summary(m_alt_2) # Rhat and ESS'es look good
summary(m_alt_3) # Rhat and ESS'es look good
