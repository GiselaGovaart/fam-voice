...
```
In our priors, we specified a mean of 3.5 and an SD of 20. In these plots and the model summary, we see that the mean of the MMR is estimated at 3.40 and is normally distributed, and the SD at 16.5. The effects on the slope were specified at ```normal(0,10)``` which also seems to be reasonably estimated. 
...
We expect that infants discriminate a vowel contrast better from the training voice when this speaker’s voice was familiar during the training, shown by a more negative mismatch response (MMR; our dependent variable, see below) in the FamTrain group than in the UnFamTrain group condition in the TrainVoiceTest (speaker 1). <br />
...
4. For both groups and speakers together: if nrSpeakersDaily is higher, MMR is larger. <br />
...
We will use the following code to compute the BFs through the following plots:
```
