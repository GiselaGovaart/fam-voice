...
```
In our priors, we specified a mean of 3.5 and an SD of 20. In these plots and the model summary, we see that the mean of the MMR is estimated at 3.49, and the SD at 16.
```{r}
...
There were 50 divergent transitions after warmup. However, the model seems to have converged, since the Rhat and ESS values are good.

We can now check the values the model comes up with for the simulated data:
b_Intercept is 4.48 (in the data it was simulated as 5)
