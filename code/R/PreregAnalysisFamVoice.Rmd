...
```
In our priors, we specified a mean of 3.5 and an SD of 20. In these plots and the model summary, we see that the mean of the MMR is estimated at 3.44, and the SD at 15.75.
```{r}
...
We run the model with more iterations (20000) to ensure full convergence:
...
There were 80 divergent transitions after warmup. However, the model seems to have converged, since the Rhat and ESS values are good.

We can now check the values the model comes up with for the simulated data:
b_Intercept is 4.68 (in the data it was simulated as 5)
...
sigma is estimated at 12.16.

These effects for TestSpeaker and Group seem a bit low, considering our simulated effects of 5. However, this is actually because of our contrast matrix:

```{r}
contrasts(dat_acq$TestSpeaker) 
```
This contrast matrix is derived from dividing -1 and 1 (sum coding) by the square root of 2. The standardization ensures that the contrast has a variance of 1, meaning that the different contrasts are independent (orthogonal) of each other in case of multiple contrasts. This makes it easier to compare outcomes across different contexts of measurements. This however also means that the coefficients are not easily interpretable in their original units anymore. So to check whether the model has a more or less sensible estimate for the effects of TestSpeaker and Group, we run the model again, with deviation coding of -0.5 and 0.5. In this case, the coding represents the difference between TestSpeaker1 and TestSpeaker2, and Group-fam and Group-unfam:
...
postpredcheck_sumcontrast_acq_m <- readRDS(here("data", "model_output", "04_model_posteriorpredcheck_sumcontrast_acq.rds"))
summary(postpredcheck_sumcontrast_acq_m)

```
Here we see that the model is actually perfectly able to retrieve the data:
b_Intercept is 5.01 (in the data it was simulated as 5)
It retrieves an effect of 4.70 for TestSpeaker (in the data it was simulated as 5)
It retrieves an effect of 4.66 for Group (in the data it was simulated as 5)
It also hallucinates some minor effects for the covariates
sigma is estimated at 12.40 (simulated as 15)

This model has 33 divergent transitions. However, Rhat and the ESS values are good.
...
Now we continue checking our model with the model that uses the coding we will use in the final analysis:
```{r}
...
# Setting orthogonal contrasts, such that the priors for the different contrasts we compare 
contrasts(dat_acq$TestSpeaker) <- contr.equalprior
contrasts(dat_acq$Group) <- contr.equalprior
contrasts(dat_acq$sleepState) <- contr.equalprior
