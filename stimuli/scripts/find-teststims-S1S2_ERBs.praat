...
# Set the outdirs
;outdir_anal$ = "../Recordings/" + speaker$ + "/analyses/"
;outdir_wavs$ = "../Recordings/" + speaker$ + "/syllables/normalized/testsyllables/"
...
for dir from 1 to 2
	if dir = 1
		indir$ = "../Recordings/" + speaker$ + "/syllables/orig/fi/"
	else
		indir$ = "../Recordings/" + speaker$ + "/syllables/orig/fe/"
	endif
	...
	for fileNum from 1 to numFiles
		...
		fileName_tg$ = (fileName$ - ".wav") + ".TextGrid"
		tg = Read from file: indir$ + fileName_tg$

		# Get start abnd end times of vowel from the TextGrid
		vBeg = Get start time of interval: 2, 2
		vEnd = Get end time of interval: 2, 2

		selectObject: sound
		formant = To Formant (burg): 0, 5, 5500, 0.025, 50
		f1 = Get mean: 1, vBeg, vEnd, "hertz"
		f2 = Get mean: 2, vBeg, vEnd, "hertz"
		# convert to ERBs
		f1 = hertzToErb(f1)
		f2 = hertzToErb(f2)
	...
	if dir = 1
		selectObject: table_all
		Save as tab-separated file: "../Recordings/" + speaker$ + "/analyses/table_trainingstim_fi_ERB_" + speaker$ + ".Table"
	else
		selectObject: table_all
		Save as tab-separated file: "../Recordings/" + speaker$ + "/analyses/table_trainingstim_fe_ERB_" + speaker$ + ".Table"
	endif
