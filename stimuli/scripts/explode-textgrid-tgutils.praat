...
# Adapted by Gisela Govaart, Aug. 2020

form Explode TextGrid...
  	integer Tier 1
  	boolean Preserve_times 0
	text Outdir /Users/giselagovaart/ownCloud/PhD/Experiment1_EEG/Stimuli/Recordings/Speaker2/syllables/orig/
endform
...
for i to intervals
	...
	if label$ <> ""
		...
		name$ = label$ + "_" + string$(counter)
  		Rename: name$
		Save as text file: outdir$ + name$ + ".TextGrid"
  		...
  		if sound
    		...
    		name$ = label$ + "_" + string$(counter)
  			Rename: name$
			Write to WAV file: outdir$ + name$ + ".wav"
