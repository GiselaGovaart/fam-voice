---
title: "Preregistered Analysis FamVoice REC"
output: html_document
date: "`r Sys.Date()`"
---

This is the analysis for the project "The Voice Familiarity Benefit for Infant Phoneme Recognition". Please note: the variable TestSpeaker (levels: 1, 2, 3) in this analysis document is called Test (levels: TrainVoiceTest, NovelVoiceTest, FamNonTrainVoiceTest) in the accompanying preregistration. TestSpeaker 1 corresponds to TrainVoiceTest, TestSpeaker2 corresponds to NovelVoiceTest, and TestSpeaker3 corresponds to FamNonTrainVoiceTest.
...
## Preparation: Pilot data and model checks
...
- mean = 3.5
- sigma = 20 (We add some uncertainty, so we multiply sigma by 1.25)
...
priorpredcheck_rec_m <- readRDS(here("data", "model_output", "03_model_priorpredcheck_rec.rds"))
...
We also set the contrast such that we get equal priors on the different comparisons. This is important for our Bayes Factor analysis later on. 
...
contrasts(dat_rec$TestSpeaker)
contrasts(dat_rec$Group) 
contrasts(dat_rec$sleepState) 
```
As we can see, the contrasts for the factors with two levels are the same as for deviation coding. For the contrast with three levels (TestSpeaker) the contrast is set such that it XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
postpredcheck_rec_m <- readRDS(here("data", "model_output", "04_model_posteriorpredcheck_rec.rds"))
summary(postpredcheck_rec_m)
```
The model fully converged.

We can now check the values the model comes up with for the simulated data: <br />
b_Intercept is 2.98 (in the data it was simulated as 5, and our prior was ```normal(3.5,20)```). <br />
It retrieves an effect of 3.31 for Group (we simulated the effect as 5). <br />
...
```{r}
plot(postpredcheck_rec_m) # check traces and posterior distributions
```

Here we see that the traces all look like nice hairy caterpillars, meaning that the chains have mixed (e.g., https://nicholasrjenkins.science/post/bda_in_r/bayes_with_brms/). The posterior distributions also look good (and we checked the estimates in the summary already).
...
m_alt_1 <- readRDS(here("data", "model_output", "05_model_sensitivity_altern1_rec.rds"))
m_alt_2 <- readRDS(here("data", "model_output", "05_model_sensitivity_altern2_rec.rds"))
m_alt_3 <- readRDS(here("data", "model_output", "05_model_sensitivity_altern3_rec.rds"))
...
## Analysis steps for experimental data

### Sensitivity analysis

First, we will run the same sensitivity analysis as in the last step above for our model with our experimental data.

### Parameter estimation experimental data
For the experimental data, we will use the following code for parameter estimation, assuming the same data structure as our simulated data, and the data called ```dat_exp_rec```
...
contrasts(dat_exp_rec$TestSpeaker) <- contr.equalprior_pairs
contrasts(dat_exp_rec$Group) <- contr.equalprior_pairs
contrasts(dat_exp_rec$sleepState) <- contr.equalprior_pairs
...
First, we will plot the traces, to check model convergence, with ```plot(model_rec, ask = FALSE)```. We will also check the posterior samples of the posterior predictive distribution, with
``` 
...
For our main RQ, we expect that infants discriminate a vowel contrast better in groupFam-TestSpeaker1 than in groupUnfam-TestSpeaker2, shown by a more negative mismatch response (MMR; our dependent variable) in groupFam-TestSpeaker1. <br /> 

**Hypothesis 2 (directional):** <br />
Testing the acoustic account, we expect that infants discriminate a vowel contrast better in groupUnfam-TestSpeaker3 than in groupUnfam-TestSpeaker2, shown by a more negative MMR in groupUnfam-TestSpeaker3. <br />

**Hypothesis 3 (directional):** <br />
Testing the exemplar account, we expect that infants discriminate a vowel contrast better in groupUnfam-TestSpeaker1 than in groupUnfam-TestSpeaker2, shown by a more negative MMR in groupUnfam-TestSpeaker1. <br />
...
1. groupFam-TestSpeaker1 vs. groupUnfam-TestSpeaker2 <br />
2. groupUnfam-TestSpeaker3 vs. groupUnfam-TestSpeaker2 <br />
3. groupUnfam-TestSpeaker1 vs. groupUnfam-TestSpeaker2 <br />
